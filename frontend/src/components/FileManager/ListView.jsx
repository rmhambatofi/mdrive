...
const ListView = ({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      >
        <div />
        <div className="pl-14 px-2 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-default">Name</div>
      ...
      {files.map((file) => {
        ...
        return (
          <div
            key={file.id}
            className={`items-center px-4 py-2 cursor-pointer transition-colors select-none border-b border-gray-50 dark:border-gray-700/50
              ${isSelected
            ...
            <div className="flex items-center space-x-3 min-w-0">
              ...
              <div className="min-w-0 flex-1 flex items-center space-x-2">
                {renamingFile?.id === file.id ? (
                  ...
                  <span className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate" title={file.file_name}>
                    {file.file_name}
                  </span>
                )}
                {isHovered && !isSelected && (
                  <button
                    className="flex-shrink-0 p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition"
